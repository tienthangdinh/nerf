...
def compute_accumulated_transmittance(alphas): #a collection of rays, each ray with bins, each bin vale is alpha, e.g: if only 1 ray [[0.8, 0.7, 0.9, 0.6]]
    accumulated_transmittance = torch.cumprod(alphas, 1) #cumulative product, doesnt change the size => [[0.8,0.56,0.504,0.3024]]
    return torch.cat((torch.ones((accumulated_transmittance.shape[0], 1), device=alphas.device),
                      accumulated_transmittance[:, :-1]), dim=-1) #basically only add 1 in the front and pop the last element out => [[1.0,0.8,0.56,0.504]]
...
def train(nerf_model, optimizer, scheduler, data_loader, device='cpu', hn=0, hf=1, nb_epochs=int(1e5),
    ...
    for _ in tqdm(range(nb_epochs)):
        for batch in data_loader:
            #input data and ground truth labels already prepared in correct format
...
if __name__ == '__main__':
    ...
    model = NerfModel(hidden_dim=128).to(device)
    model_optimizer = torch.optim.Adam(model.parameters(), lr=5e-4)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(model_optimizer, milestones=[2, 4, 8], gamma=0.5)
    data_loader = DataLoader(training_dataset, batch_size=1024, shuffle=True)
    training_loss = train(model, model_optimizer, scheduler, data_loader, nb_epochs=3, device=device, hn=2, hf=6, nb_bins=192, H=400,
          W=400)
    print(training_loss)